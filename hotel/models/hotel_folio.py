...
class HotelFolio(models.Model):
    ...
    def action_done(self):
        ...
        for rec in self:
            ...
            if rec.order_id.invoice_status == 'to invoice':
                raise UserError(_("Please invoice the remaining amount before checkout."))
            for line in rec.room_line_ids:
                rooms = self.env["hotel.room"].search(
                    [("product_id", "=", line.product_id.id)]
                )
                for rm in rooms:
                    rm.write({"isroom":True, "status": "available"})
    ...
    def action_confirm(self):
        for rec in self:
            ...
            for line in rec.room_line_ids:
                ...
                for rm in rooms:
                    rm.write({"isroom":False, "status": "occupied"})
...
class HotelFolioLine(models.Model):
    ...
    monthly_rental = fields.Boolean("Monthly")
    ...
    @api.onchange("checkin_date", "checkout_date","monthly_rental")
    def _onchange_checkin_checkout_dates(self):
        ...
        if self.checkin_date and self.checkout_date:
            ...
            difference = relativedelta.relativedelta(self.checkout_date, self.checkin_date)

            if self.monthly_rental:
                myduration = difference.months + (difference.days/30)
            else:
                sec_dur = dur.seconds
